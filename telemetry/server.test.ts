...
test("collector accepts gzip, drops all request metadata, rejects bad origins and oversized decompression", async () => {
  const stored: Buffer[] = [];
  const server = collector(async body => { stored.push(body); });
  await new Promise<void>(resolve => server.listen(0, "127.0.0.1", resolve));
  const address = server.address() as { port: number };
  const url = `http://127.0.0.1:${address.port}/replays`;
  try {
    const headers = { Origin: "https://www.learnc.dev", "Content-Type": "application/gzip", "X-Forwarded-For": "203.0.113.9", Cookie: "identity=private", "User-Agent": "private-browser", Referer: "https://private.example/" };
...
test("storage failure returns a bounded error without exposing content", async () => {
  ...
  const address = server.address() as { port: number };
  try {
    const result = await fetch(`http://127.0.0.1:${address.port}/replays`, { method: "POST", headers: { Origin: "https://learnc.dev", "Content-Type": "application/json" }, body: JSON.stringify(replay()) });
