...
export function setReplaySharing(enabled) {
    localStorage.setItem(preferenceKey, enabled ? "on" : "off");
    window.dispatchEvent(new Event("replay-preference-change"));
}
export function startLevelReplay(root) {
    const level = location.pathname.split("/").pop() || "";
    if (started || !REPLAY_ENDPOINT || !replaySharingEnabled() || !/^\d+-[a-z0-9-]+\.html$/.test(level))
        return;
    started = true;
    // Setup happens after the lesson's initial synchronous render, before input.
    queueMicrotask(() => {
        try {
            recordLevel(root, level);
        }
        catch { /* Telemetry must never break a lesson. */ }
    });
}
function recordLevel(root, level) {
    ...
    const emit = (type, data) => {
        if (live)
            worker.postMessage({ kind: "event", event: { t: Math.round(performance.now() - clock), type, data } });
    };
    const serialize = (node) => {
        if (node.nodeType === Node.TEXT_NODE)
            return { id: id(node), text: node.textContent || "" };
        if (!(node instanceof HTMLElement) || !REPLAY_TAGS.has(node.localName) || node.closest("[data-replay-ignore]"))
            return null;
        const attrs = {};
        ...
        const result = { id: id(node), tag: node.localName, attrs,
    ...
    const recordMutations = (records) => {
        ...
        try {
            ...
            for (const node of changed) {
                ...
                while (parent && !changed.has(parent))
    ...
    worker.addEventListener("message", (event) => { if (event.data?.kind === "stop")
        stop(); });
    ...
    listen(root, "input", (event) => {
        ...
        if (!(target instanceof HTMLElement) || target.closest("[data-replay-ignore]"))
            return;
    ...
    listen(root, "click", (event) => {
        ...
        const button = target.closest("button[data-role$='-check']");
    ...
    listen(root, "focusin", (event) => {
        if (event.target instanceof HTMLElement)
            emit("focus", { target: id(event.target) });
    });
    let scrollTimer = 0;
    listen(document, "scroll", (event) => {
        if (scrollTimer)
            return;
        const target = event.target;
        if (target !== document && (!(target instanceof HTMLElement) || !root.contains(target)))
            return;
        scrollTimer = window.setTimeout(() => {
            scrollTimer = 0;
            try {
                if (target === document)
                    emit("scroll", { target: 0, x: Math.round(scrollX), y: Math.round(scrollY) });
                else if (target instanceof HTMLElement)
                    emit("scroll", { target: id(target), x: Math.round(target.scrollLeft), y: Math.round(target.scrollTop) });
            }
            catch {
                stop();
            }
    ...
    listen(window, "pagehide", (event) => { if (!event.persisted)
        stop(); });
    const checkPreference = () => { if (!replaySharingEnabled())
        stop(); };
