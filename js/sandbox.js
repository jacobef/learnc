...
{
    const { $, randAddr, vbox, createSimpleSimulator, applyOtherNames, updateStepperTopControls, confettiRain, } = window.MB;
    ...
    function parseExpression(src = "") {
        ...
        if (!tokens.length)
            return { error: "Enter an expression to evaluate." };
        let idx = 0;
        ...
        const expr = parseEquality();
        if (!expr)
            return { error: "Expression needs a value." };
        if (idx < tokens.length)
            return { error: "That expression is not valid here." };
        return { expr };
    }
    ...
    function evaluateExpression(expr, state) {
        ...
        if ("error" in parsed)
            return { error: parsed.error, kind: "compile" };
        ...
        if ("error" in evaluated)
            return evaluated;
        const resultType = makePointerType(evaluated.base || "int", Number.isFinite(evaluated.depth) ? evaluated.depth : 0);
        return {
            result: {
    ...
    function getStatementContext(lines, boundary) {
        ...
        const atStatementStart = currentIsMulti && boundary === currentRange.startLine;
        return {
            statementMap,
            currentRange,
            prevRange,
