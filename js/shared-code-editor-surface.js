...
function normalizedDecorationsForLine(line, decorations) {
    ...
    const boundaries = Array.from(new Set(normalized.flatMap((decoration) => [decoration.startCol, decoration.endCol]))).sort((left, right) => left - right);
    const out = [];
    for (let index = 0; index + 1 < boundaries.length; index += 1) {
        const startCol = boundaries[index];
        const endCol = boundaries[index + 1];
        const winner = normalized
            .filter((decoration) => decoration.startCol <= startCol && decoration.endCol >= endCol)
