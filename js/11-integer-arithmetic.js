...
createProgramTemplate({
    ...
    steps: [
        ...
        },
        {
            code: "int c = b+1 == 4;\n",
            editable: false,
            instructions: '$c{x == y} evaluates to 1 if x and y have equal values, or 0 if they don\'t. $c{==} has lower precedence than addition and subtraction.\n\nThe name of this operation is "equality".',
        },
        {
            code: "int d = b == 58;\n",
            editable: false,
        ...
        },
        {
            ...
            hints: (ctx) => {
                if (ctx.basicHintTopicIs("value", "h")) {
                    return "The order of the operations used in this line is, from highest to lowest: parenthesis, division ($c{/}), subtraction ($c{-}), equality ($c{==}). Also recall that equality is left-associative, so $c{x == y == z} is parsed as $c{(x == y) == z}.";
                }
                return ctx.basicHint;
            },
        },
        {
            ...
            hints: (ctx) => {
                if (ctx.basicHintTopicIs("value", "h")) {
                    ...
                    if (h.value === "0") {
                        return "You're probably parsing it either by simply evaluating left-to-right, like $c{(((9/2)/2)-3)/7}, or as if subtraction had higher precedence than division, like $c{((9/2)/(2-3))/7}. Division has higher precedence than subtraction, so it should be parsed as $c{((9/2)/2)-(3/7)}.";
                    }
                    if (h.value === "1") {
