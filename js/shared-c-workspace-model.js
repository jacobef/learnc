...
export function resolveCBoxAliases(boxes) {
    ...
    const byAddress = new Map(resolved.filter((box) => addressOf(box)).map((box) => [addressOf(box), box]));
...
const addressSlotsByType = new Map();
function loadAddressSlots(type, count) {
    const cached = addressSlotsByType.get(type) ?? [];
    if (cached.length >= count)
        return cached;
    const declarations = Array.from({ length: count }, (_, index) => `${type} __cboxes_address_slot_${index};`).join("\n");
...
export function allocateCWorkspaceObject(boxes, requestedType) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        for (const slot of loadAddressSlots(assumedType, slotCount)) {
            ...
            const available = occupied.every((range) => end <= range.start || start >= range.end);
