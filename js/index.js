...
import { clearAllLevelProgress, savedLevelCount, } from "./shared-progress.js";
{
    ...
    resetProgressBtn.addEventListener("click", () => {
        if (savedLevelCount() <= 0)
            return;
        const confirmed = window.confirm("Reset all saved progress? This clears every level's saved state.");
        if (!confirmed)
            return;
