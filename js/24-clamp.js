...
if (clamped < low) {
  ...
  clamped = low;
        ...
        if (ctx.behavesLike(`double clamped;
if (low <= n <= high) {
  clamped = n;
} else if (n < low) {
  clamped = low;
