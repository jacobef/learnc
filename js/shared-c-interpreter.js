...
export function runCProgram(source, addressBase = syntheticAddressBase, stdin = "") {
    ...
    try {
        ...
        outputPtr = interpreter.cboxes_run_source(inputPtr, input.length, stdinPtr, stdinInput.length, addressBase);
        ...
        const output = decoder.decode(new Uint8Array(interpreter.memory.buffer, outputPtr, outputLen));
...
export function runCFiles(files, addressBase = syntheticAddressBase, stdin = "", implicitMain = true) {
    ...
    try {
        ...
        const parsed = JSON.parse(output);
        if (!parsed.ok) {
            const diagnosticSource = files.find((file) => file.path === parsed.file)?.source ??
                files[0]?.source ??
                "";
            return {
                kind: parsed.kind,
...
export function evaluateCExpression(source, eventIndex, expression, addressBase = syntheticAddressBase, stdin = "") {
    ...
    try {
        ...
        stdinPtr = interpreter.cboxes_alloc(stdinInput.length);
        new Uint8Array(interpreter.memory.buffer).set(stdinInput, stdinPtr);
        outputPtr = interpreter.cboxes_eval_expression(sourcePtr, sourceInput.length, exprPtr, exprInput.length, Math.max(0, Math.floor(eventIndex)), stdinPtr, stdinInput.length, addressBase);
...
export function evaluateCExpressionFiles(files, eventIndex, expression, addressBase = syntheticAddressBase, stdin = "", implicitMain = true) {
    ...
    try {
        ...
        const parsed = JSON.parse(output);
        if (!parsed.ok) {
            const diagnosticSource = parsed.file === "<expression>"
                ? expression
...
export function parseCValueLiteral(literal) {
