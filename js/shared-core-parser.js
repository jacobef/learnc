...
function makeSimplePointerType(depth, base) {
...
function buildDeclType(base, opsOutward) {
    let node = { kind: "base", base };
...
export function createParserTools(opts) {
    ...
    function tokenizeProgram(src = "") {
        ...
        let line = 0;
        let col = 0;
        const startsWith = (value) => src.startsWith(value, i);
        const advanceBy = (count) => {
        ...
        const parseNumberLiteral = () => {
            ...
            if (src[j] === "0" && (src[j + 1] === "x" || src[j + 1] === "X")) {
                ...
                if (j === startHex)
                    return null;
                while (j < src.length && /[uUlL]/.test(src[j]))
                    j++;
    ...
    function parseExpressionTokens(tokens, start, { allowVars = true } = {}) {
        ...
        const parseCastType = () => {
            ...
            const targetType = makeSimplePointerType(depth, base) || `${base}${"*".repeat(depth)}`;
            return { targetType, nextIndex: at + 1 };
        };
        const isAssignmentOperator = (value) => value === "=" ||
            value === "+=" ||
            value === "-=" ||
            value === "*=" ||
            value === "/=" ||
    ...
    function parseArrayLengthTokens(tokens) {
        if (!tokens.length)
            return null;
    ...
    function parseDeclarator(tokens, start) {
        let idx = start;
        let pointerCount = 0;
        ...
        const parseDirect = (from) => {
            ...
            while (at < tokens.length && tokens[at]?.type === "sym" && tokens[at]?.value === "[") {
                ...
                if (at >= tokens.length || bracketDepth !== 0)
                    return { kind: "partial" };
    ...
    function parseDeclHead(tokens) {
        ...
        if (!eqTok || eqTok.type !== "sym" || eqTok.value !== "=")
            return { kind: "none" };
        idx++;
        if (idx >= tokens.length) {
