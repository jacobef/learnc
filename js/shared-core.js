...
(function (global) {
    ...
    function isPointerType(type = "int") {
        const { depth } = parseType(type);
        return depth != null && Number.isFinite(depth) && depth > 0;
    }
    function isRefCompatible(targetType = "int*", refType = "int") {
    ...
    function ensureStepperTopControls(codepane) {
        ...
        const measure = () => {
            ...
            if (height === 0 || viewHeight === 0)
                return;
            const needsTop = height >= viewHeight ||
    ...
    function createSimpleSimulator(opts = {}) {
        const { allowVarAssign = false, allowDeclAssign = true, allowDeclAssignVar = false, requireSourceValue = false, allowPointers = false, } = opts;
        ...
        function isDeclPrefix(tokens) {
            ...
            if (rhs.type === "sym" && rhs.value === "&") {
                if (!allowPointers || !allowDeclAssignVar)
                    return false;
                const resolvedDecl = resolveDeclType(stars, baseType);
                if (!resolvedDecl)
                    return false;
                if (!isPointerType(resolvedDecl))
                    return false;
        ...
        function isStatementPrefix(tokens, declaredNames, allowIntPrefix) {
            ...
            if (tokens.some((t) => t.type === "unknown"))
                return false;
            if (!allowPointers &&
                tokens.some((t) => t.type === "sym" && (t.value === "*" || t.value === "&")))
                return false;
            if (tokens.length === 1) {
                const t0 = tokens[0];
        ...
        function parseStatementTokens(tokens) {
            ...
            if (tokens[0].type === "kw") {
                ...
                if (rhs.kind === "expr") {
                    if (!allowDeclAssign && !allowDeclAssignVar)
                        return null;
                    if (rhs.hasVar && !allowDeclAssignVar)
                        return null;
                    if (!rhs.hasVar && !allowDeclAssign)
                        return null;
                    if (stars !== 0)
                        return null;
                    return {
        ...
        function parseDeclAssignRefFallback(tokens) {
            ...
            const name = tokens[idx].value;
            idx++;
            if (idx + 2 !== tokens.length - 1)
                return null;
            if (tokens[idx].type !== "sym" || tokens[idx].value !== "=")
                return null;
            if (tokens[idx + 1].type !== "sym" || tokens[idx + 1].value !== "&")
                return null;
            if (tokens[idx + 2].type !== "ident")
                return null;
        ...
        function validateStatement(tokens, state, seenDecl, alloc) {
            ...
            if (parsed.kind === "decl" ||
                ...
                if (!ptrBase || !Number.isFinite(ptrDepth) || ptrDepth < depth) {
                    return typeMismatchError(parsed.name, makePointerType(depth, ptrBase || "int") ||
                        `int${"*".repeat(depth)}`);
                }
                ...
                while (tokenIndex < tokens.length &&
                    ...
                    if (tok.type === "sym" && tok.value === ";") {
                        if (currentTokens.length) {
                            const result = validateStatement(currentTokens, state, seenDecl, alloc);
                            if ("error" in result) {
                                status = "invalid";
    ...
    function createStepper({ root, prevButtons = null, nextButtons = null, lines = [], nextPage = null, getBoundary, setBoundary, onBeforeChange, onAfterChange, isStepLocked, getStepBadge, getNextLabel, getNextBoundary, getPrevBoundary, endLabel, } = {}) {
        ...
        function update() {
            ...
            if (nextButtons.length) {
                ...
                const badge = !atEnd && typeof getStepBadge === "function"
    ...
    function initInstructionWatcher() {
        const seen = new Map();
        const normalizeInstruction = (text = "") => String(text)
