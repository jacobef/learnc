...
export function createSimpleSimulator() {
    ...
    function integerOverflowError(_base) {
        return makeUbError();
    }
    ...
    function lineLength(lines, lineIndex) {
        const line = lines[Math.max(0, Math.min(lines.length - 1, lineIndex))] || "";
        return line.length;
    }
    function normalizeDiagnosticRange(range, lines) {
        if (!lines.length) {
            return {
                startLine: 0,
                startCol: 0,
    ...
    function rangeFromToken(token, lines) {
    ...
    function rangeFromTokens(tokens, lines) {
        if (!tokens.length) {
            return normalizeDiagnosticRange({
                startLine: 0,
                startCol: 0,
    ...
    function rangeAtLineEnd(lines, lineIndex, colHint) {
    ...
    function makeDiagnostic(kind, message, range, lines, tip) {
        return {
            kind,
            message,
            tip,
    ...
    function compareDiagnosticLocation(left, right) {
    ...
    function findIdentifierToken(tokens, name) {
        return (tokens.find((token) => token.type === "ident" && token.value === name) || null);
    }
    function findUnmatchedDelimiterDiagnostic(tokens, lines) {
        const stack = [];
        for (const token of tokens) {
            ...
            if (!open || open.value !== expected) {
                return makeDiagnostic("compile", `This ${token.value} does not match an earlier opening bracket.`, rangeFromToken(token, lines), lines, "Check that every opening bracket has the same kind of closing bracket.");
            }
            stack.pop();
        }
        const open = stack[stack.length - 1];
        ...
        return makeDiagnostic("compile", `This ${open.value} is missing its closing ${close}.`, rangeFromToken(open, lines), lines, `Add ${close} before the statement ends.`);
    }
    function missingSemicolonDiagnostic(lines, lineNumber) {
        ...
        return makeDiagnostic("compile", "This statement needs a semicolon at the end.", rangeAtLineEnd(lines, lineIndex, lastCodeCol), lines, "Add ; after the last piece of code on this line.");
    }
    function diagnoseUnknownToken(token, lines) {
        if (token.value === "/*") {
            return makeDiagnostic("compile", "This block comment never closes.", rangeFromToken(token, lines), lines, "Add */ before the end of the program.");
        }
        if (token.value === "'") {
            return makeDiagnostic("compile", "This character literal is unfinished or invalid.", rangeFromToken(token, lines), lines, "Use one character inside single quotes, like 'a' or '\\n'.");
        }
        return makeDiagnostic("compile", `I do not know what to do with ${JSON.stringify(token.value)} here.`, rangeFromToken(token, lines), lines);
    }
    function isTypeDeclarationStart(tokens) {
    ...
    function closestKnownWord(value, candidates, maxDistance) {
        const lower = String(value || "").toLowerCase();
        let best = null;
        let bestScore = null;
        for (const candidate of candidates) {
            const distance = levenshteinDistance(lower, candidate.toLowerCase());
            const firstCharPenalty = candidate[0]?.toLowerCase() === lower[0]?.toLowerCase() ? 0 : 1;
    ...
    function looksLikeUnknownTypeDeclaration(tokens) {
        if (!tokens.length || tokens[0]?.type !== "ident")
            return false;
        if (tokens.length === 1)
            return true;
        if (tokens[1]?.type === "ident")
            return true;
        if (tokens[1]?.type === "sym" &&
            (tokens[1]?.value === "*" || tokens[1]?.value === "(")) {
            return tokens.slice(2).some((token) => token.type === "ident");
        }
        return false;
    }
    function diagnoseUnknownKeywordOrType(tokens, lines) {
        if (!tokens.length || tokens[0]?.type !== "ident")
            return null;
        const first = tokens[0];
        const keywordSuggestion = closestKnownWord(first.value, ["if", "while", "else"], 2);
        if (keywordSuggestion &&
            (tokens[1]?.value === "(" ||
                tokens[1]?.value === "{" ||
                tokens.length === 1)) {
            return makeDiagnostic("compile", `I do not know the keyword ${JSON.stringify(first.value)}. Did you mean ${JSON.stringify(keywordSuggestion)}?`, rangeFromToken(first, lines), lines);
        }
        if (!looksLikeUnknownTypeDeclaration(tokens))
            return null;
        const typeSuggestion = closestKnownWord(first.value, Array.from(DIAGNOSTIC_TYPE_KEYWORDS), 2);
        return makeDiagnostic("compile", typeSuggestion
            ? `I do not know the type name ${JSON.stringify(first.value)}. Did you mean ${JSON.stringify(typeSuggestion)}?`
            : `I do not know the type name ${JSON.stringify(first.value)}.`, rangeFromToken(first, lines), lines, "The built-in types here are int, long, char, float, double, bool, and pointer versions of those.");
    }
    function diagnoseExpressionSyntax(tokens, lines) {
        const fallback = makeDiagnostic("compile", "This statement does not make sense as written yet.", rangeFromTokens(tokens, lines), lines, "Double-check the punctuation, operators, and variable names.");
        if (!tokens.length)
            return fallback;
        const first = tokens[0];
        const last = tokens[tokens.length - 1];
        if (last.type === "sym" &&
            ...
            return makeDiagnostic("compile", `This ${last.value} needs something after it.`, rangeFromToken(last, lines), lines, "Finish the expression on the right side of the operator.");
        }
        if (last.type === "sym" &&
            (last.value === ")" || last.value === "]" || last.value === "}") &&
            tokens.length >= 2) {
            const previous = tokens[tokens.length - 2];
            if (previous.type === "sym" &&
                (DIAGNOSTIC_BINARY_OPERATORS.has(previous.value) ||
                    DIAGNOSTIC_PREFIX_OPERATORS.has(previous.value))) {
                return makeDiagnostic("compile", `This ${previous.value} needs something before the closing ${last.value}.`, rangeFromToken(previous, lines), lines);
            }
        }
        if (first.type === "sym" &&
            (first.value === ")" || first.value === "]" || first.value === "}")) {
            return makeDiagnostic("compile", `This expression cannot start with ${first.value}.`, rangeFromToken(first, lines), lines);
        }
        ...
        if (!parsed) {
            if (first.type === "sym" &&
                ...
                tokens.length === 1) {
                return makeDiagnostic("compile", `This ${first.value} needs something after it.`, rangeFromToken(first, lines), lines);
            }
            if (first.type === "sym" && first.value === "=") {
                return makeDiagnostic("compile", "An assignment needs something on the left side of =.", rangeFromToken(first, lines), lines);
            }
            return fallback;
        }
        ...
        if (unexpected.type === "ident" ||
            unexpected.type === "number" ||
            (unexpected.type === "sym" && unexpected.value === "(")) {
            return makeDiagnostic("compile", "There should be an operator before this.", rangeFromToken(unexpected, lines), lines, "For example, you might need +, -, *, /, or a semicolon.");
        }
        if (unexpected.type === "sym" &&
            DIAGNOSTIC_BINARY_OPERATORS.has(unexpected.value)) {
            return makeDiagnostic("compile", `This ${unexpected.value} needs something on its right.`, rangeFromToken(unexpected, lines), lines);
        }
        return makeDiagnostic("compile", "I got stuck while reading this expression.", rangeFromToken(unexpected, lines), lines, "Double-check the punctuation near this spot.");
    }
    function diagnoseDeclarationSyntax(tokens, lines) {
        const declHead = parseDeclHead(tokens);
        const fallback = makeDiagnostic("compile", "This declaration is malformed.", rangeFromTokens(tokens, lines), lines, "Try something like int count; or int count = 3;.");
        let typeEnd = 0;
        ...
        if (typeEnd >= tokens.length) {
            ...
            return makeDiagnostic("compile", "This declaration needs a variable name after the type.", rangeFromToken(token, lines), lines);
        }
        if (declHead.kind === "partial" && declHead.hasInitializer) {
            const eqToken = tokens.find((token) => token.type === "sym" && token.value === "=");
            return makeDiagnostic("compile", "This assignment needs a value on the right side of =.", rangeFromToken(eqToken || tokens[tokens.length - 1], lines), lines);
        }
        const hasIdentifier = tokens
            .slice(typeEnd)
            .some((token) => token.type === "ident");
        if (!hasIdentifier) {
            return makeDiagnostic("compile", "This declaration needs a variable name.", rangeFromTokens(tokens.slice(0, Math.max(typeEnd, 1)), lines), lines);
        }
        const nameIndex = tokens.findIndex((token, index) => index >= typeEnd && token.type === "ident");
        const assignTokenIndex = tokens.findIndex((token) => token.type === "sym" && token.value === "=");
        if (nameIndex >= 0 && assignTokenIndex < 0) {
            const extraName = tokens.find((token, index) => index > nameIndex && token.type === "ident");
            if (extraName) {
                return makeDiagnostic("compile", "This declaration already named one variable. I did not expect another name here.", rangeFromToken(extraName, lines), lines, "End the statement after the variable name, or rewrite the declaration.");
            }
        }
        if (assignTokenIndex >= 0) {
            const rhsTokens = tokens.slice(assignTokenIndex + 1);
            if (!rhsTokens.length) {
                return makeDiagnostic("compile", "This assignment needs a value on the right side of =.", rangeFromToken(tokens[assignTokenIndex], lines), lines);
            }
        ...
        const openBracketIndex = tokens.findIndex((token) => token.type === "sym" && token.value === "[");
        if (openBracketIndex >= 0) {
            ...
            if (closeBracketIndex < 0) {
                return makeDiagnostic("compile", "This array size is missing its closing ].", rangeFromToken(tokens[openBracketIndex], lines), lines);
            }
            const lengthTokens = tokens.slice(openBracketIndex + 1, closeBracketIndex);
            if (!lengthTokens.length) {
                return makeDiagnostic("compile", "This array size needs a positive whole number.", rangeFromToken(tokens[openBracketIndex], lines), lines);
            }
            const parsedLength = parseExpressionTokens(lengthTokens, 0, {
                allowVars: false,
            });
            if (!parsedLength || parsedLength.nextIndex !== lengthTokens.length) {
                return makeDiagnostic("compile", "This array size needs a positive whole number.", rangeFromTokens(lengthTokens, lines), lines, "Use 1 or larger inside the brackets.");
            }
            const evaluatedLength = evaluateExpression(parsedLength.expr, [], {
                requireValue: true,
            });
            if ("error" in evaluatedLength ||
        ...
        if (last.type === "sym" && last.value === "=") {
            return makeDiagnostic("compile", "This assignment needs a value on the right side of =.", rangeFromToken(last, lines), lines);
        }
        return fallback;
    }
    function diagnoseConditionSyntax(tokens, keyword, lines) {
        const fallback = makeDiagnostic("compile", `This ${keyword} statement is malformed.`, rangeFromTokens(tokens, lines), lines, `Write it like ${keyword} (condition).`);
        if (tokens.length === 1) {
            return makeDiagnostic("compile", `This ${keyword} needs a condition in parentheses.`, rangeFromToken(tokens[0], lines), lines, `Write ${keyword} (condition).`);
        }
        if (tokens[1]?.type !== "sym" || tokens[1]?.value !== "(") {
            return makeDiagnostic("compile", `Put the ${keyword} condition inside parentheses.`, rangeFromToken(tokens[0], lines), lines, `Write ${keyword} (condition).`);
        }
        const headerEnd = controlHeaderEndIndex(tokens);
        if (headerEnd < 0) {
            return makeDiagnostic("compile", "This condition is missing a closing ).", rangeFromToken(tokens[1], lines), lines);
        }
        const exprTokens = tokens.slice(2, headerEnd);
        if (!exprTokens.length) {
            return makeDiagnostic("compile", `${keyword} needs a condition between the parentheses.`, rangeFromToken(tokens[1], lines), lines);
        }
    ...
    function diagnoseStatementSyntax(tokens, lines) {
        if (!tokens.length) {
            return makeDiagnostic("compile", "This statement is empty.", normalizeDiagnosticRange({ startLine: 0, startCol: 0, endLine: 0, endCol: 1 }, lines), lines);
        }
        ...
        if (unknownKeywordOrType)
            return unknownKeywordOrType;
    ...
    function findFirstUndeclaredVariable(expr, declared) {
        let missing = null;
        walkExpr(expr, (node) => {
            if (node.kind !== "var")
                return false;
    ...
    function evaluateExpressionRaw(expr, state, opts = {}) {
        ...
        const arraysByName = (() => {
            ...
            for (const box of state) {
                ...
                if (linearIndex == null || linearIndex < 0)
                    continue;
        ...
        function assignIntoTarget(target, source) {
            ...
            const targetType = makePointerType(target.depth, target.base || "int", normalizeArrayDims(target.pointeeArrayDims), target.pointeeInnerDepth) || "int";
        ...
        function evalNode(node) {
            ...
            if (node.kind === "subscript") {
                ...
                if (pointer.pointeeArrayDims.length > 0) {
                    ...
                    const targetPointeeInnerDepth = normalizePointeeInnerDepth(target.pointeeInnerDepth ?? targetParsedType.pointeeInnerDepth, targetParsedType.depth, normalizeArrayDims(target.pointeeArrayDims));
    ...
    function evaluateExpressionText(expr, state, opts = {}) {
        ...
        const resultType = evaluated.isArray
            ? arrayExpressionType(evaluated) ||
                makePointerType(evaluated.depth, evaluated.base || "int", normalizeArrayDims(evaluated.pointeeArrayDims), evaluated.pointeeInnerDepth)
            : makePointerType(evaluated.depth, evaluated.base || "int", normalizeArrayDims(evaluated.pointeeArrayDims), evaluated.pointeeInnerDepth);
    ...
    function validateAssignmentExpr(state, targetType, expr) {
    ...
    function resolveAssignmentTarget(state, lhs) {
        ...
        const targetType = makePointerType(depth, base || "int", normalizeArrayDims(evaluated.pointeeArrayDims), evaluated.pointeeInnerDepth) || "int";
    ...
    function hasArrayRoot(state, name) {
        return state.some((box) => box.arrayRoot === name);
    }
    function findNamedBox(state, name) {
        return state.find((box) => box.name === name) || null;
    }
    function findFirstUninitializedRead(expr, state) {
        const visit = (node, readContext) => {
            if (node.kind === "num")
                return null;
            if (node.kind === "var") {
                if (!readContext)
                    return null;
                if (hasArrayRoot(state, node.name))
                    return null;
    ...
    function findFirstUninitializedTargetDependency(expr, state, requiresCurrentValue) {
        if (expr.kind === "var") {
            if (!requiresCurrentValue)
                return null;
        ...
        if (expr.kind === "subscript") {
            return (findFirstUninitializedRead(expr.left, state) ||
                findFirstUninitializedRead(expr.index, state));
        }
        return findFirstUninitializedRead(expr, state);
    }
    function exprIsAssignable(expr) {
        return (expr.kind === "var" ||
            expr.kind === "subscript" ||
            (expr.kind === "unary" && expr.op === "*"));
    }
    function findTopLevelAssignmentOperatorIndex(tokens) {
        let parenDepth = 0;
        let bracketDepth = 0;
    ...
    function findDivisionByZero(expr, state) {
        let found = false;
        walkExpr(expr, (node) => {
    ...
    function findInvalidDereference(expr, state) {
        let issue = null;
        const emptyLines = [""];
        walkExpr(expr, (node) => {
            ...
            if (pointerValue === "") {
                issue = {
                    ...
                    range: normalizeDiagnosticRange({ startLine: 0, startCol: 0, endLine: 0, endCol: 1 }, emptyLines),
                };
                return true;
            }
            try {
    ...
    function expressionFailureDiagnostic(tokens, expr, state, declared, lines, opts = {}) {
        const { targetType = "double", statementRange = rangeFromTokens(tokens, lines), } = opts;
        const missing = findFirstUndeclaredVariable(expr, declared);
        if (missing) {
            const token = findIdentifierToken(tokens, missing);
            return makeDiagnostic("compile", `I do not know what ${JSON.stringify(missing)} is yet.`, token ? rangeFromToken(token, lines) : statementRange, lines, "Declare the variable before you use it.");
        }
        const uninitialized = findFirstUninitializedRead(expr, state);
        if (uninitialized) {
            const token = findIdentifierToken(tokens, uninitialized);
            return makeDiagnostic("ub", `${JSON.stringify(uninitialized)} exists, but it does not have a value yet.`, token ? rangeFromToken(token, lines) : statementRange, lines, "Assign to it before you read from it.");
        }
        if (findDivisionByZero(expr, state)) {
            return makeDiagnostic("ub", "This divides by 0, which is undefined.", statementRange, lines, "Make sure the value on the right side is not zero.");
        }
        const dereferenceIssue = findInvalidDereference(expr, state);
        if (dereferenceIssue) {
            return makeDiagnostic(dereferenceIssue.kind, dereferenceIssue.message, statementRange, lines, dereferenceIssue.tip);
        }
        const evaluated = evaluateExpressionRaw(expr, cloneBoxes(state));
        const converted = convertAssignmentValue(evaluated, targetType, requireSourceValue);
        if ("kind" in converted && converted.kind === "type-mismatch") {
            return makeDiagnostic("compile", `This expression has type ${converted.expectedType}, but the target expects ${targetType}.`, statementRange, lines, "The types on both sides need to match here.");
        }
        if ("error" in converted) {
            return makeDiagnostic(converted.kind, converted.kind === "ub"
                ? "This expression reaches undefined behavior with the current values."
                : "This expression does not work with the current variable types.", statementRange, lines, converted.kind === "ub"
        ...
        if (isEvalError(evaluated)) {
            return makeDiagnostic(evaluated.kind, evaluated.kind === "ub"
                ? "This expression reaches undefined behavior with the current values."
                : "This expression does not work with the current variable types.", statementRange, lines);
        }
        return null;
    }
    function diagnoseWritableTarget(tokens, lhs, state, declared, lines, requiresCurrentValue) {
        const targetRange = rangeFromTokens(tokens, lines);
        const missing = findFirstUndeclaredVariable(lhs, declared);
        if (missing) {
            const token = findIdentifierToken(tokens, missing);
            return {
                diagnostic: makeDiagnostic("compile", `I do not know what ${JSON.stringify(missing)} is yet.`, token ? rangeFromToken(token, lines) : targetRange, lines, "Declare the variable before you use it."),
            };
        }
        const uninitialized = findFirstUninitializedTargetDependency(lhs, state, requiresCurrentValue);
        if (uninitialized) {
            const token = findIdentifierToken(tokens, uninitialized);
            return {
                diagnostic: makeDiagnostic("ub", `${JSON.stringify(uninitialized)} does not have a usable value yet.`, token ? rangeFromToken(token, lines) : targetRange, lines, "Initialize it before you use it as part of an assignment target."),
            };
        }
        if (!exprIsAssignable(lhs)) {
            return {
                diagnostic: makeDiagnostic("compile", "The left side of this assignment must be something you can store into.", targetRange, lines, "Use a variable, an array element, or *pointer on the left side."),
            };
        }
        const resolved = resolveAssignmentTarget(cloneBoxes(state), lhs);
        if ("error" in resolved) {
            return {
                diagnostic: makeDiagnostic(resolved.kind, resolved.kind === "ub"
                    ? "This assignment target is invalid with the current values."
                    : "I cannot assign into this target.", targetRange, lines),
            };
        }
        return resolved;
    }
    function validateStatement(tokens, state, seenDecl, alloc) {
        ...
        if (parsed.kind === "decl" || parsed.kind === "declAssign") {
            ...
            if (parsed.op === "=") {
                ...
                if ("error" in resolved)
                    return resolved;
                const err = validateAssignmentExpr(state, resolved.target.type, parsed.rhs);
                if (err)
                    return err;
            }
    ...
    function parseControlStatementMaps(parts, opts = {}) {
        ...
        const fallbackLastLine = parts.length > 0 ? parts[parts.length - 1].endLine : 0;
    ...
    function getStatementContext(lines, boundary) {
        ...
        const isMultiLine = currentRange !== null && currentRange.endLine > currentRange.startLine;
        const midStatement = isMultiLine &&
    ...
    function executeProgramParts(parts, opts) {
        ...
        const fallbackEndLine = parts.length > 0 ? parts[parts.length - 1].endLine : -1;
    ...
    const normalizeStop = (stop, max) => stop === undefined ? null : Math.max(0, Math.min(max, stop));
    const normalizeStopSteps = (stopSteps) => stopSteps === undefined ? null : Math.max(0, stopSteps);
    ...
    function diagnoseProgram(text, opts = {}) {
        const lines = text.split(/\r?\n/);
        const tokens = tokenizeProgram(text);
        const parts = splitStatements(tokens);
        const candidates = [];
        ...
        while (i < parts.length) {
            if (executedParts >= maxExecutedParts) {
                ...
                return [
                    makeDiagnostic("compile", "This program never settles into a finished state.", rangeFromTokens(rangePart.tokens, lines), lines, "Check for a loop that never stops."),
                ];
            }
            executedParts += 1;
            const part = parts[i];
            ...
            if (parsed.kind === "if") {
                ...
                if (!block) {
                    ...
                    if (!nextPart) {
                        return [
                            makeDiagnostic("compile", "This if statement needs a body after it.", statementRange, lines),
                        ];
                    }
                    if (!isBracePart(nextPart, "{") && isDeclarationPart(nextPart)) {
                        return [
                            makeDiagnostic("compile", "A declaration directly under if needs braces.", statementRange, lines, "Write if (...) { int x = ...; }"),
                        ];
                    }
                    return [
                        makeDiagnostic("compile", "This if statement is incomplete.", statementRange, lines),
                    ];
                }
                const conditionTokens = part.tokens.slice(2, controlHeaderEndIndex(part.tokens));
                const conditionDiagnostic = expressionFailureDiagnostic(conditionTokens, parsed.expr, state, declared, lines, { statementRange });
                if (conditionDiagnostic)
                    return [conditionDiagnostic];
                const condition = evaluateCondition(parsed.expr, state);
                if ("error" in condition) {
                    return [
                        makeDiagnostic(condition.kind, condition.kind === "ub"
                            ? "This if condition has undefined behavior with the current values."
                            : "This if condition does not compile.", statementRange, lines),
                    ];
                }
                ifDecisions.set(block.headerIndex, condition.value);
                if (condition.value) {
            ...
            if (parsed.kind === "while") {
                ...
                if (!block) {
                    ...
                    if (!nextPart) {
                        return [
                            makeDiagnostic("compile", "This while loop needs a body after it.", statementRange, lines),
                        ];
                    }
                    if (!isBracePart(nextPart, "{") && isDeclarationPart(nextPart)) {
                        return [
                            makeDiagnostic("compile", "A declaration directly under while needs braces.", statementRange, lines, "Write while (...) { int x = ...; }"),
                        ];
                    }
                    return [
                        makeDiagnostic("compile", "This while loop is incomplete.", statementRange, lines),
                    ];
                }
                const conditionTokens = part.tokens.slice(2, controlHeaderEndIndex(part.tokens));
                const conditionDiagnostic = expressionFailureDiagnostic(conditionTokens, parsed.expr, state, declared, lines, { statementRange });
                ...
                if ("error" in condition) {
                    return [
                        makeDiagnostic(condition.kind, condition.kind === "ub"
                            ...
                            : "This while condition does not compile.", statementRange, lines),
                    ];
                }
                if (condition.value) {
                    whileStack.push(block);
            ...
            if (parsed.kind === "else") {
                ...
                if (!block) {
                    return [
                        makeDiagnostic("compile", "This else does not have a matching if.", statementRange, lines),
                    ];
                }
                const decision = ifDecisions.get(block.headerIndex);
                if (decision == null) {
                    return [
                        makeDiagnostic("compile", "This else appears before its matching if can be understood.", statementRange, lines),
                    ];
                }
                if (decision) {
                    i = continueCompletedWhileLoops(block.afterIndex);
                ...
                if (!elseEntry) {
                    return [
                        makeDiagnostic("compile", "This else needs a body after it.", statementRange, lines),
                    ];
                }
                if (elseEntry.pushScope)
                    scopes.push(new Set());
            ...
            if (parsed.kind === "blockEnd") {
                ...
                if (popped.error) {
                    return [
                        makeDiagnostic("compile", "This } does not match any earlier {.", statementRange, lines),
                    ];
                }
                state = popped.state;
                i = continueCompletedWhileLoops(i + 1);
            ...
            if (isDeclLikeStatement(parsed) && declared.has(parsed.name)) {
                ...
                return [
                    makeDiagnostic("compile", `You already declared ${JSON.stringify(parsed.name)} in this scope.`, nameToken ? rangeFromToken(nameToken, lines) : statementRange, lines, "Reuse the existing variable or pick a new name."),
                ];
            }
            if (parsed.kind === "declAssign") {
                const assignIndex = findTopLevelAssignmentOperatorIndex(part.tokens);
                const rhsTokens = assignIndex >= 0 ? part.tokens.slice(assignIndex + 1) : part.tokens;
                const rhsRange = rangeFromTokens(rhsTokens.length ? rhsTokens : part.tokens, lines);
                const diagnostic = expressionFailureDiagnostic(rhsTokens.length ? rhsTokens : part.tokens, parsed.expr, state, declared, lines, {
                ...
                const assignIndex = findTopLevelAssignmentOperatorIndex(part.tokens);
                const lhsTokens = assignIndex > 0 ? part.tokens.slice(0, assignIndex) : part.tokens;
                const rhsTokens = assignIndex >= 0 ? part.tokens.slice(assignIndex + 1) : part.tokens;
                const target = diagnoseWritableTarget(lhsTokens.length ? lhsTokens : part.tokens, parsed.lhs, state, declared, lines, parsed.op !== "=");
                if ("diagnostic" in target)
                    return [target.diagnostic];
                if (parsed.op === "=") {
                    const rhsRange = rangeFromTokens(rhsTokens.length ? rhsTokens : part.tokens, lines);
                    const diagnostic = expressionFailureDiagnostic(rhsTokens.length ? rhsTokens : part.tokens, parsed.rhs, state, declared, lines, {
                    ...
                    const diagnostic = expressionFailureDiagnostic(part.tokens, {
                        kind: "assign",
                        op: parsed.op,
                        left: parsed.lhs,
                        right: parsed.rhs,
                    }, state, declared, lines, {
                        targetType: target.targetType,
                        statementRange,
                    });
                    if (diagnostic)
                ...
                const diagnostic = expressionFailureDiagnostic(part.tokens, parsed.expr, state, declared, lines, { statementRange });
                if (diagnostic)
                    return [diagnostic];
            }
            const next = applyStatement(state, parsed, {
            ...
            if (!next) {
                return [
                    makeDiagnostic("compile", "This statement does not work as written.", statementRange, lines),
                ];
            }
            if (isDeclLikeStatement(parsed)) {
                addDeclaredNames(scopes, declared, parsed.declaredNames);
