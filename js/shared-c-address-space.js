...
export function createSyntheticAddressBase() {
    ...
    const firstBase = Math.ceil(MIN_ADDRESS_BASE / ADDRESS_ALIGNMENT) * ADDRESS_ALIGNMENT;
    const baseCount = Math.floor((MAX_ADDRESS_BASE - firstBase) / ADDRESS_ALIGNMENT) + 1;
    return firstBase + (random[0] % baseCount) * ADDRESS_ALIGNMENT;
}
/** Keeps a page's displayed addresses stable across reloads. */
function loadPageAddressBase() {
