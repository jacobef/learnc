...
function lastAssignmentOperatorBefore(text: string, beforeColumn: number): number {
...
function destinationRangeBeforeExpression(
  lines: string[],
  primary: ProgramDiagnosticRange,
): ProgramDiagnosticRange | null {
  for (let line = primary.startLine; line >= Math.max(0, primary.startLine - 6); line -= 1) {
    const text = lineAt(lines, line);
    if (line < primary.startLine && /[;{}]\s*$/.test(text)) return null;
    ...
    if (!left || /^return\b/.test(left)) return null;
...
function linkedDiagnosticMessageParts(
  message: string,
  hasDestination: boolean,
): ProgramDiagnosticMessagePart[] | undefined {
  const links: Array<{ start: number; end: number; annotationId: string }> = [];
  ...
  const parts: ProgramDiagnosticMessagePart[] = [];
  let cursor = 0;
  for (const link of links) {
    if (link.start < cursor) continue;
    if (link.start > cursor) parts.push({ text: message.slice(cursor, link.start) });
    parts.push({
...
type CTagKeyword = "struct" | "union" | "enum";

function declarationLooksLikeUnknownType(
  line: string,
  column: number,
  name: string,
): boolean {
  if (line.slice(column, column + name.length) !== name) return false;
...
function sourceBeforePosition(lines: string[], line: number, column: number): string {
  return [
    ...lines.slice(0, line),
...
function visibleTagKeywordBefore(
  lines: string[],
  line: number,
  column: number,
  name: string,
  ...
  let keyword: CTagKeyword | null = null;
  for (const match of source.matchAll(pattern)) {
    keyword = match[1] as CTagKeyword;
  }
  return keyword;
}

function unknownTypeNameDiagnostic(
  lines: string[],
  line: number,
  column: number,
  name: string,
...
type FriendlyDiagnosticContext = {
  message: string;
  source: string;
  line: number;
  col: number;
  ...
  safeLine: number;
  text: string;
  textAtDiagnostic: string;
  atEnd: boolean;
  diagnosticOffset: number;
};

function syntaxDiagnostic(context: FriendlyDiagnosticContext): FriendlyDiagnostic | null {
  const { message, source, line, col, lines, safeLine, text, textAtDiagnostic, atEnd, diagnosticOffset } = context;
  ...
  if (/^expected '}', found /.test(message)) {
    if (
      /^expected '}', found (?:number|identifier|a character literal|a string literal)/.test(message)
...
function declarationDiagnostic(context: FriendlyDiagnosticContext): FriendlyDiagnostic | null {
  const { message, col, lines, safeLine, text } = context;
...
function executionDiagnostic(context: FriendlyDiagnosticContext): FriendlyDiagnostic | null {
  const { message, col, lines, safeLine, text } = context;
  ...
  if (/^attempt to modify a string literal or other read-only object$/.test(message)) {
    ...
    if (name) {
      const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const declaration = new RegExp(
        `\\bchar\\s*\\*\\s*${escapedName}\\s*=\\s*(\"(?:\\\\.|[^\"\\\\])*\")`,
      ).exec(lines.slice(0, safeLine + 1).join("\n"));
      if (declaration) {
        return {
...
function fallbackDiagnostic(context: FriendlyDiagnosticContext): FriendlyDiagnostic | null {
  const { message, col, lines, safeLine, text, textAtDiagnostic } = context;
  ...
  const mismatchedArrayString =
    /^cannot initialize (.+) with (an? .+ string literal); use a brace-enclosed list or change the array element type to (.+)$/.exec(message);
  ...
  const arrayConversion =
    /^cannot convert (.+) to (.+); an array of this type must use (.+)$/.exec(message);
  if (arrayConversion) {
    return {
...
function friendlyDiagnosticFor(
  ...
  const context: FriendlyDiagnosticContext = {
    message,
    source,
    line,
    col,
  ...
  return (
    syntaxDiagnostic(context)
    ?? declarationDiagnostic(context)
    ?? executionDiagnostic(context)
    ?? fallbackDiagnostic(context)
    ?? { message }
  );
}

export function rustDiagnostic(raw: CInterpreterError, source: string): ProgramDiagnostic {
  ...
  if (!hasFriendlyRange && raw.endLine != null && raw.endColumn != null) {
    ...
    endCol = Math.max(
      0,
      Math.min(lineEndColumn(lines, endLine), Math.floor(Number(raw.endColumn))),
    );
  ...
  const rawAnnotations = (raw.annotations || [])
    ...
    .map((annotation) => {
      ...
      const endCol = Math.max(
        endLine === startLine ? startCol + 1 : 0,
        Math.floor(Number(annotation.endColumn ?? startCol + 1)),
      );
      return {
  ...
  const rawDestinationRange = rawAnnotations.find(
    (annotation) => annotation.id === "destination",
  )?.range;
