...
export interface UpdateCodeSurfaceOptions {
  ...
  lineNumberClasses?: Map<number, string[]>;
  decorations?: CodeDecoration[];
}

function textValueForLine(line: string): string {
  return line === "" ? " " : line;
}
...
function normalizedDecorationsForLine(
  line: string,
  decorations: CodeDecoration[],
): CodeDecoration[] {
...
function appendLineContent(
  ...
  for (const decoration of normalizedDecorations) {
    ...
    if (start > cursor) {
      lineEl.appendChild(
        document.createTextNode(textValueForLine(line.slice(cursor, start))),
      );
    }
...
export function ensureCodeSurfaceElements(
  editor: HTMLTextAreaElement | null,
): CodeSurfaceElements {
  if (!editor?.parentElement) {
    return { highlightEl: null, measureEl: null };
  }
  let highlightEl = editor.parentElement.querySelector<HTMLPreElement>(
  ...
  let measureEl = editor.parentElement.querySelector<HTMLDivElement>(
    ".code-textarea-measure",
  );
  if (!measureEl) {
...
export function updateCodeSurface({
  editor,
  lineNumbers = null,
  highlightEl = null,
  measureEl = null,
  ...
  lineNumberClasses,
  decorations = [],
}: UpdateCodeSurfaceOptions): { wraps: number[]; lineHeight: number } {
  ...
  if (highlightEl) {
    ...
    safeLines.forEach((line, index) => {
      ...
      if (extraLineClasses.length) lineEl.classList.add(...extraLineClasses);
  ...
  if (lineNumbers) {
    ...
    safeLines.forEach((_line, index) => {
      ...
      if (extraLineNumberClasses.length) num.classList.add(...extraLineNumberClasses);
...
export function bindCodeEditorTabKey(
  editor: HTMLTextAreaElement | null,
): void {
  if (!editor || editor.dataset.codeTabBound === "1") return;
  ...
  editor.addEventListener("keydown", (event) => {
    ...
    if (event.shiftKey) {
      ...
      if (!removedPerLine.some((count) => count > 0)) return;
      ...
      if (start === end) {
        const nextPos = Math.max(
          lineStart,
          start - Math.min(removedPerLine[0] || 0, start - lineStart),
        );
