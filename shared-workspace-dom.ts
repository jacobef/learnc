...
export function vbox({
  ...
  `);
  if (stateName) node.dataset.stateName = stateName;
...
export function disableBoxEditing(root: Element | null) {
  if (!root) return;
  root
    .querySelectorAll(
...
export function removeBoxDeleteButtons(root?: Element | null) {
...
export function readBoxState(root: Element): BoxState {
...
export function applyOtherNames(
  root: Element | null,
  opts: {
    onToggle?: (() => void) | null;
...
export function findArrayObjectBoxesForResult(
  ...
  const withResultTypeInfo = (boxes: BoxState[]) =>
    boxes.map((box) => ({ ...box, typeInfo: result.typeInfo ?? box.typeInfo ?? null }));
  for (const item of grouped) {
    ...
    if (subset) return withResultTypeInfo(subset);
  }
  return null;
}
...
function aggregatePath(box: BoxState): string[] {
...
function appendScalarStateBox(
  ...
  if (box.defaultAddressType) node.dataset.defaultAddressType = box.defaultAddressType;
  if (box.expectedAddress) node.dataset.expectedAddress = box.expectedAddress;
...
function makeAggregateBox(
  root: BoxState,
  descendants: BoxState[],
  opts: {
    editable: boolean;
    deletable: boolean;
    allowNameEdit: boolean | null;
  ...
  const statusNode = node.querySelector(".aggregate-status") as HTMLElement;
  statusNode.textContent = status;
  statusNode.classList.toggle("hidden", !status);

  const membersNode = node.querySelector(".aggregate-members") as HTMLElement;
  ...
  for (const member of directMembers) {
    ...
    if (member.typeInfo?.kind === "array") {
      ...
      const group = groupStateObjects(arrayBoxes).find(
        (item): item is GroupedArrayObject => item.kind === "array",
      );
      if (group) {
        membersNode.appendChild(
          makeArrayBox(group, {
            editable: opts.editable,
...
export function appendStateObjects(
  container: HTMLElement,
  boxes: BoxState[],
  opts: {
    editable?: boolean;
  ...
  const originalIndex = new Map(source.map((box, index) => [box, index] as const));
  const aggregateRoots = source.filter(
    (box) => !!box.aggregateKind && !box.aggregateRoot,
  );
  const aggregateRootNames = new Set(aggregateRoots.map((box) => box.name));
  const ordinaryBoxes = source.filter(
    (box) =>
      !aggregateRoots.includes(box) &&
      !(box.aggregateRoot && aggregateRootNames.has(box.aggregateRoot)),
  );
  const renderItems: Array<
    | { kind: "aggregate"; root: BoxState; descendants: BoxState[]; index: number }
    | { kind: "ordinary"; object: GroupedStateObject; index: number }
  ...
  for (const object of groupStateObjects(ordinaryBoxes)) {
    const index =
      object.kind === "scalar"
        ...
        : Math.min(
            ...object.entries.map(
              (entry) => originalIndex.get(entry.box) ?? entry.index,
            ),
          );
  ...
  for (const item of renderItems) {
    if (item.kind === "aggregate") {
      container.appendChild(
        makeAggregateBox(item.root, item.descendants, {
          editable,
          deletable,
          allowNameEdit,
          allowTypeEdit,
