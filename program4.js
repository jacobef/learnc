(function(MB){
  const {
    $, randAddr, vbox, renderCodePaneEditable, readEditableCodeLines,
    createHintController, createStepper, pulseNextButton, isEmptyVal,
    parseSimpleStatement, applySimpleStatement, flashStatus, el
  } = MB;
  ...
  $('#p4-check').onclick=()=>{
    ...
    const expected=p4.expected;
    const match = Array.isArray(state) && state.length===expected.length &&
      state.every((b,i)=>b.name===expected[i].name && b.type===expected[i].type && String(b.value||'')===String(expected[i].value||''));
