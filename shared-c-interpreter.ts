...
function writeU64(memory: WebAssembly.Memory, ptr: number, value: bigint) {
  ...
  return rawState.map((item) => {
    ...
    return {
      ...
      aggregatePath: Array.isArray(box.aggregatePath)
        ...
        : null,
      aggregateKind:
        box.aggregateKind === "struct" || box.aggregateKind === "union"
          ? box.aggregateKind
          : null,
...
function normalizeTypeInfo(raw: unknown): CTypeInfo {
  ...
  const normalizeHelpNode = (value: unknown, depth = 0): CTypeInfo["helpTree"] => {
    ...
    return {
      kind: allowedKinds.has(nodeKind)
        ? nodeKind as "pointer" | "array" | "function" | "type"
        : "type",
